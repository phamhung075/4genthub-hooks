...
class ContextInjectionConfig:
    ...
    def __post_init__(self):
        ...
        if not explicitly_set:
            ...
            test_modules = any(
                mod in sys.modules for mod in ["pytest", "_pytest", "unittest"]
            )
            # Check environment variables
            test_env = any(
                env in os.environ for env in ["PYTEST_CURRENT_TEST", "TEST_MODE"]
            )
            ...
            logger.debug(
                f"Test mode detection: modules={test_modules}, env={test_env}, argv={test_argv}, cwd={test_cwd}, result={self.test_mode}"
            )
        else:
...
class ContextRelevanceDetector:
    ...
    def is_context_relevant(
        ...
        if "actions" in trigger_config:
            ...
            if action in trigger_config["actions"]:
                context_reqs = self._get_mcp_context_requirements(
        ...
        if "file_extensions" in trigger_config:
            ...
            if file_path and any(
                file_path.endswith(ext) for ext in trigger_config["file_extensions"]
            ):
                context_reqs = self._get_file_context_requirements(
            ...
            if pattern:
                ...
                if tool_name == "Glob":
                    ...
                    pattern_match = any(
                        ...
                        for p in trigger_config["patterns"]
                        if "*." in p
                    )
                else:
                    # For Grep and other tools, use substring matching
                    pattern_match = any(
                        p in pattern.lower() for p in trigger_config["patterns"]
                    )

                if pattern_match:
                    context_reqs = self._get_search_context_requirements(
                        pattern, tool_input
                    )
                    return True, priority, context_reqs
    ...
    def _get_mcp_context_requirements(
...
class MCPContextQuery:
    ...
    async def query_context(self, context_requirements: dict) -> dict | None:
        ...
        try:
            if query_type == "mcp_operation":
                context_data = await self._query_mcp_operation_context(
    ...
    async def _query_mcp_operation_context(self, requirements: dict) -> dict | None:
        ...
        if "task_id" in requirements:
            ...
            if task_context:
                ...
                logger.warning(
                    f"Failed to retrieve required task context for {requirements['task_id']}"
                )
                return None

        # Get git branch context if needed
        if "git_branch_id" in requirements:
            branch_context = await self._get_branch_context(
                requirements["git_branch_id"]
            )
            if branch_context:
    ...
    async def _query_search_context(self, requirements: dict) -> dict | None:
        ...
        if any(
            keyword in pattern.lower() for keyword in ["todo", "fixme", "bug", "error"]
        ):
    ...
    async def _get_task_context(self, task_id: str) -> dict | None:
        ...
        try:
            result = self.mcp_client.make_request(
                "/mcp/manage_task",
                {"action": "get", "task_id": task_id, "include_context": True},
            )
    ...
    async def _get_branch_context(self, git_branch_id: str) -> dict | None:
        ...
        try:
            result = self.mcp_client.make_request(
                "/mcp/manage_git_branch",
                {"action": "get", "git_branch_id": git_branch_id},
            )
    ...
    async def _get_file_related_tasks(self, file_path: str) -> list[dict] | None:
        ...
        try:
            ...
            result = self.mcp_client.make_request(
                "/mcp/manage_task", {"action": "search", "query": filename, "limit": 5}
            )
    ...
    async def _get_pattern_related_tasks(self, pattern: str) -> list[dict] | None:
        ...
        try:
            result = self.mcp_client.make_request(
                "/mcp/manage_task", {"action": "search", "query": pattern, "limit": 3}
            )
...
class ContextInjector:
    ...
    async def inject_context(
        ...
        try:
            ...
            is_relevant, priority, context_reqs = self.detector.is_context_relevant(
            ...
            logger.info(
                f"Context injection triggered for {tool_name} (priority: {priority})"
            )
        ...
        if "related_tasks" in context_data:
            ...
            if tasks:
                ...
                for task in tasks[:3]:  # Limit to 3 most relevant
                    lines.append(
                        f"- **{task.get('title', 'N/A')}** ({task.get('status', 'N/A')})"
                    )
                lines.append("")
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    asyncio.run, injector.inject_context(tool_name, tool_input)
                )
