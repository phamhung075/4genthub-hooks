...
class InstallationValidator:
        ...
        for suggestion in self.validation_results["repair_suggestions"]:
            ...
            try:
                if suggestion["action"] == "run_setup":
                    ...
                    if self._repair_create_file(
                        suggestion["file_path"], suggestion.get("content", "")
                    ):
                        repairs_successful += 1
                        print("✅ Repair successful")
                    else:
    ...
    def _validate_core_structure(self):
        ...
        self._add_success(
            "core_structure", "project_root", f"Found: {self.project_root}"
        )
    ...
    def _validate_configuration(self):
        ...
        if not settings_file.exists():
            ...
            self._add_repair_suggestion(
                "run_setup", "Generate settings.json configuration"
            )
            return
        ...
        try:
            ...
            if "{{PROJECT_ROOT}}" in settings_str:
                self._add_error(
                    "configuration",
                    "PROJECT_ROOT placeholder not replaced",
                    "Run setup_hooks.py to replace placeholders",
                )
                self._add_repair_suggestion(
                    "run_setup", "Replace PROJECT_ROOT placeholder"
                )
            else:
                self._add_success(
                    "configuration",
            ...
            self._add_error(
                "configuration",
                f"Error reading settings.json: {e}",
    ...
    def _validate_mcp_json(self, mcp_file: Path):
        ...
        try:
            ...
            self._add_error(
                "configuration", f"Invalid JSON in .mcp.json: {e}", "Fix JSON syntax"
            )
        except Exception as e:
            self._add_warning(
                "configuration",
                f"Could not validate .mcp.json: {e}",
    ...
    def _validate_environment(self):
        ...
        if is_valid:
            self._add_success(
                "environment", "environment_valid", "Environment validation passed"
            )
        else:
            for issue in issues:
                self._add_warning(
                    "environment", issue, "Check environment requirements"
                )
        ...
        if (
            ...
            and python["version_info"]["minor"] >= 7
        ):
            self._add_success(
                "environment",
                "python_version",
            ...
            self._add_error(
                "environment", "Python version too old", "Python 3.7+ required"
            )
    ...
    def _validate_dependencies(self):
        ...
        if not UTILS_AVAILABLE:
            self._add_warning(
                "dependencies",
                "Dependency manager not available",
        ...
        if missing_count == 0:
            self._add_success(
                "dependencies", "all_dependencies", "All dependencies available"
            )
        else:
            self._add_warning(
                "dependencies",
                f"{missing_count} packages missing",
    ...
    def _validate_hook_execution(self):
        ...
        if not execute_hook.exists():
            self._add_error(
                "hook_execution", "execute_hook.py missing", "Reinstall hooks"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self._add_warning(
                    "hook_execution",
                    "Hook executor returned non-zero",
        ...
        for hook_name in test_hooks:
            ...
            if hook_path.exists():
                try:
                    ...
                    self._add_warning(
                        "hook_execution",
                        f"Could not validate {hook_name}: {e}",
    ...
    def _validate_integration(self):
        ...
        if settings_file.exists():
            self._add_success(
                "integration", "settings_location", "settings.json in correct location"
            )
            ...
            if gitignore_file.exists():
                try:
                    ...
                    if ".claude/settings.json" in gitignore_content:
                        ...
                        self._add_warning(
                            "integration",
                            "settings.json not in .gitignore",
        ...
        for filename in protection_files:
            ...
            if file_path.exists():
                ...
                if sample_path.exists():
                    self._add_warning(
                        "integration",
                        f"Hook protection not configured: {filename}",
    ...
    def _generate_report(self):
        ...
        warning_count = len(self.validation_results["warnings"])
        success_count = sum(
            len(tests) for tests in self.validation_results["tests"].values()
        )
    ...
    def _add_warning(self, category: str, message: str, suggestion: str = ""):
        ...
        self.validation_results["warnings"].append(
            {"category": category, "message": message, "suggestion": suggestion}
        )
    ...
    def _add_repair_suggestion(self, action: str, description: str, **kwargs):
        ...
        suggestion = {"action": action, "description": description}
